...
export const RobotErrorLog = ({ item, currentLayout }) => {
  ...
  useEffect(() => {
    ...
    const myHeaderTable = document.querySelector(
      ".table-header"
    ) as HTMLElement;
    ...
    if (currentLayout) {
      setMaxHeight(
        maxRow * currentLayout.h - sumStatic - headerCard - headerTable
      );
    } else {
  ...
  return (
    <Card style={{ height: "100%", position: "relative" }}>
      <CardHeader className={"drag-header"}>Robot ErrorLog</CardHeader>
      <CardBody>
        <Table
